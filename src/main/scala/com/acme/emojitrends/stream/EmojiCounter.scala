...
class EmojiCounter(dao: RedisEmojiCounterDao, configuration: Configuration)
                  (implicit val materializer: Materializer,
                   executionContext: ExecutionContext,
                   loggingAdapter: LoggingAdapter) extends StatusAdapter {
  ...
  private val extractEmojis = Flow[Status]
    .map { status =>
      EmojiParser.extractEmojis(status.getText).asScala
    }
    .async
    .filter { emojis =>
      emojis.nonEmpty
